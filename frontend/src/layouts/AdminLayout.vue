      ...
      <div class="sidebar-footer">
        <template v-if="!collapsed">
          <div class="footer-user-row">
            ...
            <router-link to="/operator/profil" class="footer-icon-btn" title="Profil Ayarları" @click="mobileOpen = false">
        ...
        <template v-else>
          <div class="footer-collapsed">
            ...
            <router-link to="/operator/profil" class="footer-icon-btn-sm" title="Profil Ayarları">
...
.sidebar {
  width: 260px;
  flex-shrink: 0;
  background: linear-gradient(180deg, #0f172a 0%, #0f172a 70%, #1a103a 100%);
  display: flex;
...
.brand-icon {
  ...
  justify-content: center;
  color: white;
  box-shadow: 0 4px 12px rgba(155,145,245,0.3);
  flex-shrink: 0;
  transition: transform 0.2s var(--ease), box-shadow 0.2s var(--ease);
}

.brand-link:hover .brand-icon { transform: translateY(-1px) scale(1.04); box-shadow: 0 6px 16px rgba(155,145,245,0.4); }
...
.nav-item--active {
  background: rgba(155,145,245,0.12);
  color: #c4beff;
  font-weight: 600;
}
...
.nav-divider { height: 1px; background: rgba(255,255,255,0.06); margin: 8px 4px 4px; flex-shrink: 0; }
...
.nav-section-label {
  ...
  opacity: 1;
  max-height: 24px;
  transition: opacity 0.12s ease, max-height 0.22s ease, padding 0.22s ease;
}
...
.footer-badge { padding: 1px 7px; background: rgba(155,145,245,0.2); color: #c4beff; border-radius: 50px; font-size: 10px; font-weight: 700; }
...
@media (max-width: 768px) {
  ...
  /* Mobile: show all text regardless of collapsed state */
  ...
  .sidebar .nav-section-label { opacity: 1 !important; max-height: 24px !important; padding: 4px 14px 2px !important; }
}
</style>
